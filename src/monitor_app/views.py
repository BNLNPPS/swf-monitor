...
@api_view(['POST'])
def lxr_slash_command(request):
    """Handle /lxr slash commands from Mattermost.

    Subcommands:
        ident <symbol>         — Find definitions and references
        search <pattern>       — Ripgrep search across all repos
        source <path> [L1 L2]  — Read a source file (optional line range)
        list [path]            — Browse directory contents
        help                   — This help text
    """
    from decouple import config as decouple_config

    expected_token = decouple_config('MATTERMOST_LXR_SLASH_TOKEN', default='')
    received_token = request.data.get('token', '')
    ...
    LXR_BASE = 'https://eic-code-browser.sdcc.bnl.gov/lxr'
    OMIT_PREFIXES = ['/juggler/', '/include/EICrecon/']
    ...
    try:
        ...
        def _fetch(path, params=None):
            resp = httpx.get(f"{LXR_BASE}/{path.lstrip('/')}", params=params, timeout=30)
        ...
        def _omit(path):
            return any(path.startswith(p) for p in OMIT_PREFIXES)
        ...
        if subcmd == 'help' or not text:
            return JsonResponse({'text': _lxr_slash_help()})

        if subcmd == 'ident':
            if not arg:
                return JsonResponse({'text': 'Usage: `/lxr ident <symbol>`'})
            soup = _fetch('ident', {'_i': arg, 'v': 'master'})
            lines = []
            ...
            if def_table:
                for row in def_table.find_all('tr'):
                    ...
                    if len(cells) >= 4:
                        ...
                        dtype = _txt(cells[0])
                        member = _txt(cells[1])
                        line_no = _txt(cells[3])
                        label = dtype
                        if member:
                            label += f' ({member})'
                        lines.append(f"| {label} | [{fpath}:{line_no}]({_link(fpath, line_no)}) |")
                ...
                for row in ref_table.find_all('tr'):
                    ...
                    if len(cells) >= 2:
                        ...
                        if _omit(current_file):
                            continue
                        ln = _txt(cells[1])
                        if ln and current_file:
                            refs.append(f"[{current_file}:{ln}]({_link(current_file, ln)})")

            out = [f"#### `{arg}` — Definitions"]
            if lines:
            ...
            if refs:
                out.append(f'\n**References** ({len(refs)}): ' + ', '.join(refs[:30]))
                if len(refs) > 30:
                    out.append(f'... and {len(refs) - 30} more')
            return JsonResponse({'text': '\n'.join(out)})

        if subcmd == 'search':
            if not arg:
                return JsonResponse({'text': 'Usage: `/lxr search <pattern> [fileglob]`'})
            search_parts = arg.split(None, 1)
            pattern = search_parts[0]
            file_pattern = search_parts[1] if len(search_parts) > 1 else ''
            params = {'_string': pattern, 'v': 'master', '_casesensitive': '1'}
            if file_pattern:
            ...
            current_file = ''
            if ref_table:
                tbody = ref_table.find('tbody')
                rows = tbody.find_all('tr') if tbody else ref_table.find_all('tr')
                for row in rows:
                    cells = row.find_all('td')
                    if len(cells) < 3:
                        continue
                    ...
                    if _omit('/' + current_file):
                        continue
                    ln = _txt(cells[1])
                    match_text = cells[2].get_text().strip() if cells[2] else ''
                    if ln and current_file:
                        results.append(f"| [{current_file}:{ln}]({_link('/' + current_file, ln)}) | {match_text[:100]} |")
                    if len(results) >= 30:
                        break
            out = [f"#### Search: `{pattern}`" + (f" in `{file_pattern}`" if file_pattern else '')]
            if count_text:
                out.append(count_text)
            if results:
        ...
        if subcmd == 'source':
            if not arg:
                return JsonResponse({'text': 'Usage: `/lxr source <path> [start_line end_line]`'})
            source_parts = arg.split()
            path = source_parts[0]
            start = int(source_parts[1]) if len(source_parts) > 1 else 1
            end = int(source_parts[2]) if len(source_parts) > 2 else 0
            soup = _fetch(f'source/{path.lstrip("/")}')
            pre = soup.find('pre', class_='filecontent')
            if not pre:
                return JsonResponse({'text': f'File not found: `{path}`'})
            file_lines = []
            for anchor in pre.find_all('a', class_='fline'):
                n = int(anchor.get_text())
                parts_content = []
                for sib in anchor.next_siblings:
                    if hasattr(sib, 'name') and sib.name == 'a' and 'fline' in (sib.get('class') or []):
                        break
                    parts_content.append(sib.get_text() if hasattr(sib, 'get_text') else str(sib))
                file_lines.append((n, ''.join(parts_content).rstrip('\n')))
            if end > 0:
                file_lines = [(n, c) for n, c in file_lines if start <= n <= end]
            elif start > 1:
                file_lines = [(n, c) for n, c in file_lines if n >= start]
            if not file_lines:
                return JsonResponse({'text': f'No lines in that range for `{path}`'})
            width = len(str(file_lines[-1][0]))
            code = '\n'.join(f"{n:{width}d}  {c}" for n, c in file_lines[:200])
            link = _link('/' + path.lstrip('/'), str(file_lines[0][0]))
            header = f"[{path}]({link}) (lines {file_lines[0][0]}-{file_lines[-1][0]})"
            return JsonResponse({'text': f"{header}\n```\n{code}\n```"})

        if subcmd in ('list', 'ls'):
            path = arg.strip('/') + '/' if arg.strip('/') else ''
            soup = _fetch(f'source/{path}')
            table = soup.find('table', class_='dircontent')
            if not table:
                return JsonResponse({'text': f'Not a directory: `{arg}`'})
            entries = []
            ...
            if tbody:
                for row in tbody.find_all('tr'):
                    ...
                    if name == 'Parent directory':
                        continue
                    size = _txt(cells[2])
                    is_dir = name.endswith('/') or 'dirfolder' in str(cells[1])
                    icon = '📁' if is_dir else '📄'
                    size_str = '' if size == '-' else f' ({size})'
                    entries.append(f"| {icon} | [{name}]({LXR_BASE}/source/{path}{name}) | {size_str} |")
            out = [f"#### /{path}" if path else '#### / (top-level repos)']
            if entries:
                out.append('| | Name | Size |')
                out.append('|-|------|------|')
                out.extend(entries[:50])
            else:
                out.append('Empty directory.')
            return JsonResponse({'text': '\n'.join(out)})

        return JsonResponse({'text': f"Unknown subcommand `{subcmd}`. Try `/lxr help`"})

    except Exception as e:
        logger.exception(f"LXR slash command error: {subcmd} {arg}")
        return JsonResponse({'text': f"Error: {e}"})


def _lxr_slash_help():
    return """#### /lxr — EIC Code Browser
| Command | Description |
|---------|-------------|
| `/lxr ident <symbol>` | Find definitions and references for a class, function, variable |
| `/lxr search <pattern> [fileglob]` | Ripgrep search across all 55+ repos |
| `/lxr source <path> [start end]` | Read source file (optional line range) |
| `/lxr list [path]` | Browse directory contents |
| `/lxr help` | This help |

Browse: https://eic-code-browser.sdcc.bnl.gov/lxr/source"""
