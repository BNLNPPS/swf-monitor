...
def _analysis_shares():
    ...
    with connections['panda'].cursor() as cur:
        cur.execute(
            "SELECT name, value, prodsourcelabel "
            "FROM doma_panda.global_shares ORDER BY value DESC")
        shares = [dict(zip(['name', 'value', 'labels'], r))
                  for r in cur.fetchall()]
        cur.execute(
            "SELECT gshare, "
            "sum(hs) FILTER (WHERE jobstatus IN "
            "('sent','running','starting')) AS executing_hs, "
