...
CALIBRATION_CACHE_KEY = 'panda:queue-census:calibration:v1'
...
def _calibration():
    ...
    production jobs' median and p90 walltime in hours and their count,
    and the peak number of concurrently running jobs of any type (a
    sweep over start and end times, the currently running jobs
    included)."""
    ...
    with connections['panda'].cursor() as cursor:
        ...
        for site, n, med, p90 in cursor.fetchall():
...
def queue_census(refresh=False):
    ...
    names = set(live) | set(calib_queues) | set(gate)
    queues = {}
