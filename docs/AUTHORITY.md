...
### The roles: PAC and Ops

| Field | Values | Written by |
|---|---|---|
| `pac` | unset, true | a person, in the User admin page, and nothing else |
| `ops` | unset, true | a person, in the User admin page, and nothing else |

Roles are flags beside the access level, held together or apart. A
physics analysis coordinator (PAC) may set request priorities. Operations
administers accounts on the User admin page, sets priorities, and holds
authority to act on its own, like `basic`. The former `rights: ops` rung
is read as the operations role, so a record written before the roles
existed keeps what it had, and clearing the role on such a record lowers
the rung to `basic`; the User admin page offers `read` and `basic` as the
level and the two roles as checkboxes.

```
may act           =  rights != 'read'  and  (eic is true  or  rights == 'basic'  or  ops is true)
may set priority  =  may act  and  (pac is true  or  ops is true)
```
...
`is_ops`, `set_role` (`set_pac`, `set_ops`) and `PRIORITY_REFUSAL` live in
`monitor_app/authority.py`; the roles are written through the same endpoint
as `rights`, `POST /api/user-rights/ {"username": ..., "pac": true|false,
"ops": true|false}`, so the sign-in sweep's endpoint cannot touch them
either.
...
**User admin**, under the System menu, visible to staff and to accounts
holding the operations role. It lists accounts with their GitHub login,
origin, `eic`, `rights`, the PAC and Ops roles, and last sign-in, and it is
where `rights` and the roles are set. `eic` is display-only there: it
