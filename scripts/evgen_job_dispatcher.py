...
# PILOT_EVENTRANGECHANNEL; the node harness shipped with the payload
# (payload/es/es_harness.py) takes them, keeps ES_SLOTS slot containers
# fed and reports each range's outcome; this mode starts it and writes
# the job report from its summary. The pilot runs an Event Service
# payload outside the job's container, so the harness runs here, in the
# pilot's environment, and starts the task's image itself, one long-lived
# container per slot. The harness's work tree sits one level above
# runGen's workDir, which runGen removes when the payload exits, before
# the pilot tars the receipts the ranges were reported with.
ES_WORK_DIR = "es_work"
...
def run_event_service(csv_base, stamp, workdir, channel):
    ...
    with the job report carrying every range it did or failed."""
    ...
    cmd = [sys.executable, os.path.join(workdir, PAYLOAD_SUBDIR, "es", "es_harness.py"),
           ...
           "--env", f"CANARY_LIFETIME_S={CANARY_LIFETIME_S}",
           "--env", f"PANDAID={os.environ.get('PANDAID', '')}"]
    for key in ("ES_DEADLINE_S", "ES_MARGIN_S"):
