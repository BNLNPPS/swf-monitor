...
2.  **MCP App (`mcp_app`):** This app provides the real-time communication layer.
    *   **WebSocket Service:** Built with Django Channels, this service uses WebSockets to handle asynchronous, bidirectional communication with agents. Currently, its primary function is to receive and process `heartbeat` messages.
